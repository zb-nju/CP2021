...
int Program::run(int *eip) {
  ...
  int _start[] = {
    ...
    while (true) {
        ...
        dbgs("crs:  ");
        for (auto i = 0u; i < ctrl_regs.size(); i++) dbgs(" %08x", ctrl_regs[i]);
        dbgs("\n");
#endif
        ...
        switch ((Opc)opc) {
            ...
            case Opc::la: {
                int to = *eip++;
                int from = *eip++;
                stack.back().at(to) = ((stack.back().begin() + from) - memory.begin()) * 4;
                dbgs("%p: la %d, %d\n", oldeip, to, from);
            } break;
            case Opc::ld: {
                int to = *eip++;
            ...
            case Opc::div: {
                ...
                if (stack.back().at(rhs) == 0) {
                    ...
                    return -1;
#endif
                } else if (stack.back().at(lhs) == INT_MIN && stack.back().at(rhs) == -1) {
#ifndef IGNORE_CERTAIN_EXCEPTION
            ...
            case Opc::br: {
                ...
                uint64_t ptrlo = *eip++;
                uint64_t ptrhi = *eip++;
                dbgs("%p: cond %d br %p\n", oldeip, cond, lohi_to_ptr<void>(ptrlo, ptrhi));
                if (cond) {
                    eip = lohi_to_ptr<int>(ptrlo, ptrhi);
                }
            } break;
...
bool Compiler::handle_assign(Program *prog, const TokenList &toks) {
    dbgs("at %s, %d\n", __func__, __LINE__);
    // ID := (ID|NUM|&ID)
    if (toks.size() != 3 || toks.at(0).at(0) == '*' || toks.at(1) != ":=") return false;
...
bool Compiler::handle_takeaddr(Program *prog, const TokenList &toks) {
    dbgs("at %s, %d\n", __func__, __LINE__);
    // ID := &ID
    if (toks.size() != 3 || toks.at(1) != ":=" || toks.at(2).at(0) != '&') return false;
    auto &x = toks.at(0);
...
bool Compiler::handle_deref(Program *prog, const TokenList &toks) {
    ...
    // ID := *ID
    /* stmt assign */
    if (toks.size() != 3 || toks.at(0).at(0) == '*' || toks.at(2).at(0) != '*' || toks.at(1) != ":=") return false;
...
bool Compiler::handle_branch(Program *prog, const TokenList &toks) {
    dbgs("at %s, %d\n", __func__, __LINE__);
    // IF a < b GOTO label
    if (toks.size() != 6 || toks.at(4) != "GOTO") return false;
    ...
    static std::map<std::string, Opc> s2op{
        {"<", Opc::slt}, {">", Opc::sgt}, {"<=", Opc::sle}, {">=", Opc::sge}, {"==", Opc::seq}, {"!=", Opc::sne},
    };
...
std::unique_ptr<Program> Compiler::compile(std::istream &is) {
    ...
    while ((is.peek(), is.good())) {
        ...
        prog->gen_inst(Opc::helper, ptr_lo((void *)log_curir), ptr_hi((void *)log_curir), 3, ptr_lo(ir), ptr_hi(ir),
                       lineno);
#endif
        ...
        if (tokens.size() == 0) {
            ...
            std::vector<bool (Compiler::*)(Program *, const TokenList &)> handlers{
                &Compiler::handle_assign, &Compiler::handle_arith,        &Compiler::handle_takeaddr,
                &Compiler::handle_deref,  &Compiler::handle_deref_assign,
            };
