...
int main(int argc, char** argv) {
	...
	Operand t1 = newOperand(TEMP_VAR_OP, "t1");
	Operand v1 = newOperand(VARIABLE_OP, "v1");
	Operand t2 = newOperand(TEMP_VAR_OP, "t2");
	Operand t3 = newOperand(TEMP_VAR_OP, "t3");
	Operand t6 = newOperand(TEMP_VAR_OP, "t6");
	...
	a[4] = newIR(ASSIGN_IR, t2, newOperand(CONSTANT_OP, 0));
	...
	a[10] = newIR(ASSIGN_IR, t3, newOperand(CONSTANT_OP, 1));
	a[11] = newIR(WRITE_IR, t3);
	a[12] = newIR(GOTO_IR, label3);
	a[13] = newIR(LABEL_IR, label2);
	a[14] = newIR(ASSIGN_IR, t6, newOperand(CONSTANT_OP, -1));
